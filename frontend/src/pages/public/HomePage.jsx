...
export default function HomePage() {
  useDocumentMeta({
    title: "Siddipet Real Estate | Properties in Siddipet District",
  ...
  const highlights = [
    ["Verified", "Listings checked by the office"],
    ["Offline", "Visits and paperwork handled locally"],
    ["Local", "Focused on Siddipet district"],
  ];
    ...
    <div className={`mandal-marquee mt-4 overflow-hidden py-2 ${direction === "right" ? "mandal-marquee-reverse" : ""}`}>
      <div className="mandal-marquee-track flex w-max gap-3">
        {[...rowMandals, ...rowMandals].map((mandal, index) => (
          ...
          >
            <span>{mandal}</span>
            <span className="mt-3 block text-xs font-extrabold uppercase text-slate-400 group-hover:text-brand-600">Browse listings</span>
          </Link>
        ))}
      </div>
    </div>
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-slate-950 text-white">
        ...
        <div className="container-page relative grid min-h-[660px] items-center py-16 md:py-20">
          <div className="max-w-3xl">
            <p className="inline-flex rounded-md border border-white/20 bg-white/10 px-3 py-2 text-xs font-black uppercase text-brand-100 backdrop-blur">
              Pragnapur office, Siddipet district
            </p>
            ...
            <form className="mt-8 flex max-w-2xl flex-col gap-3 rounded-lg border border-white/20 bg-white p-3 shadow-soft sm:flex-row" onSubmit={search}>
              <input
                ...
                value={q}
              />
              <button className="btn-primary px-6 py-3 text-base" type="submit">Search properties</button>
            </form>
      ...
      <section className="container-page py-14">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <Link className="btn-secondary self-start md:self-end" to="/properties">View all listings</Link>
        </div>
        <div className="mt-4">
      ...
      <section className="bg-white/60 py-12">
        <div className="container-page">
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            ...
            <Link className="btn-secondary" to="/properties">Explore more</Link>
          </div>
      ...
      <section className="container-page py-12">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            ...
            <h2 className="section-title mt-2">Simple offline property support</h2>
          </div>
        </div>
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {[
            ["1", "Shortlist", "Search by local area, property type, and budget."],
            ["2", "Contact", "Call, WhatsApp, or send an inquiry to the office team."],
            ["3", "Visit", "Plan site visits and document checks offline with local guidance."],
          ].map(([step, title, text]) => (
